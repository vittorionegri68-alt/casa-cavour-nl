// ─────────────────────────────────────────────────────────────────────────────
// qanda.jsx — Vragen & Antwoorden voor Casa Cavour (Nederlandse versie)
//
// Een nieuwe vraag toevoegen:
// 1. Kopieer een van de onderstaande blokken
// 2. Verander id, domanda en risposta
// 3. Sla op — de Q&A verschijnt automatisch in de sectie
// ─────────────────────────────────────────────────────────────────────────────

export const qanda = [
  {
    id: "base-forli-cesena",
    domanda: "Is Casa Cavour een goede uitvalsbasis voor Forlì en Cesena?",
    risposta: "Ja, Bertinoro ligt op slechts 12 km van Forlì en 15 km van Cesena — minder dan 20 minuten rijden. Vanuit Casa Cavour bereik je beide steden moeiteloos, breng je de dag door met verkennen en keer je 's avonds terug naar de rust van de heuvels.",
  },
  {
    id: "colline-romagnole",
    domanda: "Is het eenvoudig om de heuvels van Romagna te bezoeken vanuit Casa Cavour?",
    risposta: "Casa Cavour ligt in het hart van Bertinoro, een van de mooiste dorpen in de heuvels van Romagna. Nabijgelegen dorpen zoals Polenta, Meldola en Longiano zijn op 15-20 minuten afstand. De heuvels beginnen letterlijk voor de deur.",
  },
  {
    id: "tour-cantine",
    domanda: "Is Casa Cavour geschikt voor een verblijf tijdens een wijnkelderreis?",
    risposta: "Absoluut. Bertinoro is de hoofdstad van de Romagnoolse wijn en de historische wijnkelders in de omgeving liggen op slechts enkele kilometers afstand. De volledig uitgeruste keuken laat je toe de gekochte flessen mee naar huis te nemen en diners te bereiden met lokale producten.",
  },
  {
    id: "weekend-enogastronomico",
    domanda: "Is Casa Cavour een goede keuze voor een wijn- en culinair weekend?",
    risposta: "Het is de ideale keuze. Je bevindt je in het hart van de wijn- en eetcultuur van Romagna — op slechts enkele minuten van de wijnkelders van Predappio, de trattorias van Bertinoro en de lokale boerenmarkten. De uitgeruste keuken maakt de ervaring compleet.",
  },
  {
    id: "cosa-fare-bertinoro",
    domanda: "Wat te doen in Bertinoro tijdens een weekend?",
    risposta: "Het historische centrum is in een paar uur te voet te verkennen, tussen de Rocca, de Colonna delle Anelle en het panoramische uitkijkpunt over de Povlakte. In de omgeving liggen de wijnkelders van de Wijn- en Smakenroute en de dorpen Polenta, Meldola en Longiano. Casa Cavour is de praktische uitvalsbasis om beide te combineren: een volledig uitgeruste keuken om wijn en lokale producten mee naar huis te nemen, en self check-in waarmee je terugkeertijd vrij blijft na een diner in een osteria of een wijnproeverij die uitloopt.",
  },
  {
    id: "giardino-privato",
    domanda: "Heeft Casa Cavour een privétuin in de heuvels van Romagna?",
    risposta: "Ja. De tuin is privé en exclusief voor het appartement, niet gedeeld met andere units, met tafel en stoelen om buiten te eten. Handig voor een diner als koppel zonder reservering, om je hond vrij te laten rondlopen, of gewoon om van de Romagnoolse heuvels te genieten zonder het huis te verlaten. Het ligt op een steenworp afstand van het historische centrum van Bertinoro, dus niet geïsoleerd zoals vaak het geval is bij agriturismi met meer grond maar minder voorzieningen in de buurt.",
  },
  {
    id: "animali-ammessi",
    domanda: "Accepteert Casa Cavour huisdieren in Romagna?",
    risposta: "Ja, zonder extra kosten. De privétuin laat je hond vrij rondlopen, de uitgeruste keuken is ook handig om de maaltijden van je huisdier te regelen, en self check-in met sleutelkastje voorkomt de stress van strikte aankomsttijden na een lange autorit. De ligging in het historische centrum van Bertinoro betekent dat je direct vanaf de voordeur een wandeling kunt maken.",
  },
  {
    id: "staccare-bologna",
    domanda: "Is Casa Cavour geschikt voor wie even wil ontsnappen aan de drukte van Bologna?",
    risposta: "Perfect. Op slechts 70 km van Bologna — minder dan een uur rijden — biedt Bertinoro een volledig ander tempo. Stilte, panoramisch uitzicht over de heuvels, authentieke Romagnoolse keuken. De ontsnapping aan de stad begint zodra je aankomt.",
  },
  {
    id: "weekend-romantico",
    domanda: "Is Casa Cavour romantisch voor een weekendje weg als koppel?",
    risposta: "Ja, het is precies daarvoor ontworpen. Een privéappartement in het historische centrum van Bertinoro, met uitzicht op de heuvels, een keuken om samen te koken en het panoramische uitkijkpunt op een steenworp afstand voor de mooiste zonsondergangen van Romagna.",
  },
  {
    id: "ricaricare-energie",
    domanda: "Is Casa Cavour een goede plek om bij te komen?",
    risposta: "Dat is precies wat het is. Bertinoro beweegt op het trage ritme van de Romagnoolse heuvels — geen drukte, schone lucht, landschappen die de geest kalmeren. Casa Cavour biedt een privé, comfortabele ruimte om echt los te koppelen.",
  },
  {
    id: "smart-working",
    domanda: "Is Casa Cavour geschikt om een paar dagen op afstand te werken?",
    risposta: "Ja. Het appartement beschikt over wifi, comfortabele ruimtes en een volledig uitgeruste keuken zodat je zelfstandig je maaltijden kunt regelen. Werk met uitzicht op de Romagnoolse heuvels en na werktijd heb je een buitengewoon gebied om te verkennen.",
  },
];
